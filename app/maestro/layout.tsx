...
import { BookOpen, FileText, Plus, Lock, LogOut, Menu, X, ChevronRight, School, LayoutDashboard } from 'lucide-react'

const supabase = createClient()

const navItems = [
    { label: 'Panel Principal', href: '/maestro', icon: LayoutDashboard, exact: true },
    { label: 'Mis Cursos', href: '/maestro/curso', icon: BookOpen },
    { label: 'Crear Tarea', href: '/maestro/crear-tarea', icon: Plus },
    { label: 'Cambiar Contraseña', href: '/maestro/cambiar-password', icon: Lock },
]
...
export default function MaestroLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex">
            <a href="#main-content" className="sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2 focus:z-50 focus:px-4 focus:py-2 focus:bg-blue-600 focus:text-white focus:rounded-md focus:text-sm focus:font-medium">
            ...
            >
                <div className="flex items-center justify-between p-5 border-b border-gray-100">
                    ...
                    <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-1.5 rounded-md text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-purple-500" aria-label="Cerrar menú">
                ...
                {userName && (
                    <div className="px-4 py-3 border-b border-gray-100">
                        <p className="text-xs text-gray-500 uppercase tracking-wide font-medium mb-0.5">Maestro</p>
                            ...
                            return (
                                <li key={item.href}>
                                    <Link href={item.href} aria-current={active ? 'page' : undefined}
                                        className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors duration-150 group ${active ? 'bg-purple-50 text-purple-700' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'} focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-1`}
                                    >
                                        <item.icon className={`h-4 w-4 flex-shrink-0 ${active ? 'text-purple-600' : 'text-gray-400 group-hover:text-gray-600'}`} aria-hidden="true" />
                                        <span className="flex-1">{item.label}</span>
                                        {active && <ChevronRight className="h-3.5 w-3.5 text-purple-400" aria-hidden="true" />}
                                    </Link>
                                </li>
                            )
                        })}
            ...
            <div className="flex-1 flex flex-col min-w-0">
                <header className="lg:hidden sticky top-0 z-10 bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3">
                    <button onClick={() => setSidebarOpen(true)}
                        className="p-2 rounded-md text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-purple-500"
