...
export default function AlumnoLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex">
            <a href="#main-content" className="sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2 focus:z-50 focus:px-4 focus:py-2 bg-[#29476b] text-white focus:rounded-md focus:text-sm focus:font-medium">
            ...
            >
                <div className="flex items-center justify-between p-5 border-b border-gray-100">
                    ...
                    <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-1.5 rounded-md text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-[#29476b]" aria-label="Cerrar menú">
                            ...
                            return (
                                <li key={item.href}>
                                    <Link href={item.href} aria-current={active ? 'page' : undefined}
                                        className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors duration-150 group ${active ? 'bg-[#29476b]/10 text-[#29476b]' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'} focus:outline-none focus:ring-2 focus:ring-[#29476b] focus:ring-offset-1`}
                                    >
                                        <item.icon className={`h-4 w-4 flex-shrink-0 ${active ? 'text-[#29476b]' : 'text-gray-400 group-hover:text-gray-600'}`} aria-hidden="true" />
