...
import { BookOpen, FileText, Award, Lock, LogOut, Menu, X, ChevronRight, School, LayoutDashboard } from 'lucide-react'

const supabase = createClient()

const navItems = [
    { label: 'Mi Panel', href: '/alumno', icon: LayoutDashboard, exact: true },
    { label: 'Mis Cursos', href: '/alumno/tarea', icon: BookOpen },
    { label: 'Calificaciones', href: '/alumno/calificaciones', icon: Award },
    { label: 'Cambiar Contraseña', href: '/alumno/cambiar-password', icon: Lock },
]
...
export default function AlumnoLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        async function loadUser() {
            ...
            const { data: alumno } = await supabase.from('alumnos').select('matricula, grado, grupo').eq('user_id', user.id).single()
            if (alumno) setMatricula(`${alumno.grado} ${alumno.grupo} · Matrícula: ${alumno.matricula}`)
        }
        loadUser()
    }, [])
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex">
            <a href="#main-content" className="sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2 focus:z-50 focus:px-4 focus:py-2 focus:bg-green-600 focus:text-white focus:rounded-md focus:text-sm focus:font-medium">
            ...
            >
                <div className="flex items-center justify-between p-5 border-b border-gray-100">
                    ...
                    <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-1.5 rounded-md text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-green-500" aria-label="Cerrar menú">
                ...
                {userName && (
                    <div className="px-4 py-3 border-b border-gray-100">
                        <p className="text-xs text-gray-500 uppercase tracking-wide font-medium mb-0.5">Alumno</p>
                        <p className="text-sm font-semibold text-gray-800 truncate">{userName}</p>
                        {matricula && <p className="text-xs text-gray-500 mt-0.5">{matricula}</p>}
                    </div>
                )}
                            ...
                            return (
                                <li key={item.href}>
                                    <Link href={item.href} aria-current={active ? 'page' : undefined}
                                        className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors duration-150 group ${active ? 'bg-green-50 text-green-700' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'} focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-1`}
                                    >
                                        <item.icon className={`h-4 w-4 flex-shrink-0 ${active ? 'text-green-600' : 'text-gray-400 group-hover:text-gray-600'}`} aria-hidden="true" />
                                        <span className="flex-1">{item.label}</span>
                                        {active && <ChevronRight className="h-3.5 w-3.5 text-green-400" aria-hidden="true" />}
                                    </Link>
                                </li>
                            )
                        })}
